#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# Bitcoin Regtest Workshop — Demo B (full)
#
# Full demo: both nodes connected from the start, real-time P2P propagation.
...
FINAL_HEIGHT=$($CLI1 getblockcount 2>/dev/null || echo "?")
...
echo "     • Both nodes show the same height (${FINAL_HEIGHT} blocks)"
